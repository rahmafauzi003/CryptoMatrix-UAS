(function () {
  ...
  function updateInputLabels() {
    ...
    if (fmt === "hex") {
      ...
      els.dataInput.placeholder = mode === "encrypt"
        ? "Contoh plaintext hex: ffeeddccbbaa99887766554433221100"
        : "Contoh ciphertext hex: isi ciphertext 16 byte hasil enkripsi, misalnya dari output RuangSandi";
    } else {
      ...
      els.dataInput.placeholder = mode === "encrypt"
        ? "Contoh plaintext teks: Matrix Gate 01"
    ...
    return `
      <details id="${section.id}" class="glass rounded-[1.75rem] p-0" ${open ? "open" : ""}>
        <summary class="flex cursor-pointer items-center justify-between gap-4 p-5 sm:p-6">
          <div>
            <div class="text-xs font-black uppercase tracking-[0.18em] text-cyan-200">Bagian ${index + 1}</div>
  ...
  function renderKeyExpansion(exp) {
    ...
    const gRows = exp.details.filter(d => d.type === "g").map(d => `
      <tr class="border-t border-white/10">
        ...
        <td class="px-3 py-2 font-mono text-cyan-100">${A.wordToHex(d.result)}</td>
      </tr>`).join("");
    ...
    return `
      <details id="key-expansion" class="glass rounded-[1.75rem]" open>
        <summary class="flex cursor-pointer items-center justify-between gap-4 p-5 sm:p-6">
          <div>
            <div class="text-xs font-black uppercase tracking-[0.18em] text-fuchsia-300">Matrix Key Expansion</div>
  ...
  function renderSummary(trace) {
    ...
    return `
      <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div class="details-tag">Hasil Akhir Matrix</div>
          <h2 class="mt-3 text-2xl font-black">${action}</h2>
          <p class="mt-2 break-all font-mono text-sm leading-6 text-cyan-100">${trace.outputHex}</p>
          ${readable}
        </div>
  ...
  function renderNav(trace) {
    const items = [
      { id: "key-expansion", label: "Matrix Key Expansion" },
      ...trace.sections.map(s => ({ id: s.id, label: s.title })),
      { id: "reference", label: "Bank Konstanta" }
    ];
  ...
  function renderAll(trace) {
    ...
    copyBtn.addEventListener("click", async () => {
      try {
        ...
        showMessage("success", "Output hex sudah disalin ke clipboard.");
      } catch (_) {
        showMessage("info", "Clipboard browser tidak tersedia. Salin manual nilai hex yang tampil.");
      }
    });
    ...
    els.visualization.innerHTML = `
      <section class="empty-state">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div class="max-w-3xl">
            <div class="details-tag">Matrix Standby</div>
            <h2 class="mt-3 text-2xl font-black text-white">Masukkan payload untuk menjalankan RuangSandi AES-128</h2>
            <p class="mt-3 details-note">
              Isi payload manual, gunakan sampel RuangSandi, atau muat vector standar AES. Setelah eksekusi, sistem akan menampilkan pembentukan kunci, perubahan state setiap ronde, dan hasil akhir dalam format heksadesimal.
            </p>
          </div>
          <div class="grid w-full max-w-md gap-3 text-sm text-slate-300">
            <div class="info-card">
              <div class="info-card-label">Sampel Hex</div>
              <div class="info-card-value font-mono text-sm">ffeeddccbbaa99887766554433221100</div>
            </div>
            <div class="info-card">
              <div class="info-card-label">Kunci Sampel</div>
              <div class="info-card-value font-mono text-sm">0f0e0d0c0b0a09080706050403020100</div>
            </div>
            <div class="info-card">
              <div class="info-card-label">Sampel Teks</div>
              <div class="info-card-value text-sm">Matrix Gate 01</div>
            </div>
          </div>
        </div>
      </section>`;
    els.roundNav.innerHTML = `<a class="nav-link active" href="#controls">Console Input</a><a class="nav-link" href="#reference">Bank Konstanta</a>`;
  ...
  function run() {
    try {
      ...
      showMessage("success", `${expected} selesai diproses oleh RuangSandi. Output: ${trace.outputHex}`);
    } catch (err) {
  ...
  function fillExampleHex() {
    els.mode.value = "encrypt";
    els.inputFormat.value = "hex";
    els.dataInput.value = "ffeeddccbbaa99887766554433221100";
    els.keyInput.value = "0f0e0d0c0b0a09080706050403020100";
    updateInputLabels();
    showMessage("info", "Sampel hex RuangSandi telah dimuat. Klik 'Eksekusi Matrix' untuk memulai.");
  }

  function fillExampleText() {
    els.mode.value = "encrypt";
    els.inputFormat.value = "text";
    els.dataInput.value = "Matrix Gate 01";
    els.keyInput.value = "1f1e1d1c1b1a19181716151413121110";
    updateInputLabels();
    showMessage("info", "Sampel teks RuangSandi telah dimuat. Klik 'Eksekusi Matrix' untuk memulai.");
  }
  ...
  function renderConstants() {
    els.constantTables.innerHTML = [
      renderConstTable("S-Box Matrix", A.SBOX),
      renderConstTable("Inverse S-Box Matrix Matrix", A.INV_SBOX),
      renderRcon(),
      `<div class="grid gap-4 lg:grid-cols-2">${renderSmallMatrix("Matrix MixColumns", A.MIX_MATRIX)}${renderSmallMatrix("InvMatrix MixColumns", A.INV_MIX_MATRIX)}</div>`
    ].join("");
  }
